...
def elevation():
    ...
    try:
        ...
        # RLE-compress per-point source_tags into segments: "0-3400:PT_MDT2M,3401-6200:ES_WCS_5M"
        source_segments_parts = []
        if source_tags:
            seg_start = 0
            seg_tag = source_tags[0] or ''
            for j in range(1, len(source_tags)):
                cur = source_tags[j] or ''
                if cur != seg_tag:
                    if seg_tag:
                        source_segments_parts.append(f'{seg_start}-{j - 1}:{seg_tag}')
                    seg_start = j
                    seg_tag = cur
            if seg_tag:
                source_segments_parts.append(f'{seg_start}-{len(source_tags) - 1}:{seg_tag}')
        ...
        if source_segments_parts:
            response.headers['X-Source-Segments'] = ','.join(source_segments_parts)
        response.headers['Access-Control-Expose-Headers'] = (
            'X-Summary, X-Countries, X-Elevation-Source, X-Elevation-Sources, '
            'X-Elevation-Resolved, X-Elevation-Missing, X-Source-Segments'
        )
        return response
...
def get_coverage():
    from coverage import build_coverage_geojson
