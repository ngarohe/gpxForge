#!/usr/bin/env python3
"""Build Austria DGM5 tile index.

Default output matches server/config.py:
  server/data/austria_tiles/dgm5/index.json
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Austria DGM5 index for GPXForge")
    parser.add_argument("--input-dir", default="", help="Directory containing Austria DGM5 GeoTIFFs")
    ...
    parser.add_argument(
        "--output",
        default=str(Path(__file__).resolve().parents[1] / "data" / "austria_tiles" / "dgm5" / "index.json"),
        help="Output index path (default: server/data/austria_tiles/dgm5/index.json)",
    )
