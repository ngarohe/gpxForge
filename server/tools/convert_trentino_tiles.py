"""
Convert Trentino LIDAR .asc.zip tiles to GeoTIFF and build index.json.

Usage:
    python convert_trentino_tiles.py --src ./italy/trentino_raw --dst ./italy/trentino_tif

Steps:
  1. Read .asc directly from each .asc.zip (no disk extraction)
  2. Convert .asc (EPSG:25832 UTM 32N) to GeoTIFF with CRS tag
  3. Compute WGS84 bounding boxes
  4. Build/merge index.json for the CatalogTileProvider

The .asc files use EPSG:25832 (ETRS89 / UTM zone 32N).
Tiles are 1000×1000 cells at 0.5m = 500m × 500m per tile.
"""
import argparse
import glob
import json
...
def asc_to_geotiff_from_bytes(raw_bytes, tif_path, crs=TILE_CRS):
    ...
    # Check if tile is all nodata
    valid_mask = data != nodata
    if not np.any(valid_mask):
        return None  # skip all-nodata tiles
...
def main():
    parser = argparse.ArgumentParser(description="Convert Trentino .asc.zip tiles to GeoTIFF + index.json")
    parser.add_argument("--src", default="./italy/trentino_raw", help="Directory with .asc.zip files")
    parser.add_argument("--dst", default="./italy/trentino_tif", help="Output directory for .tif + index.json")
    ...
    # Find .asc.zip files (the inner zips extracted from STEM download)
    zips = sorted(glob.glob(os.path.join(args.src, "*_DTM.asc.zip")))
    # Also try without _DTM suffix
    if not zips:
        zips = sorted(glob.glob(os.path.join(args.src, "*.asc.zip")))
    print(f"Found {len(zips)} .asc.zip files in {args.src}")

    if not zips:
        print("No .asc.zip files found. Run download_trentino.py first.")
        sys.exit(1)
    ...
    for i, zpath in enumerate(zips):
        ...
        # Tile ID: strip _DTM.asc.zip suffix
        tile_id = zname.replace("_DTM.asc.zip", "").replace(".asc.zip", "")
        ...
        try:
            ...
            if converted % 500 == 0 or converted <= 3:
