...
from elevation_providers import (
    ...
    GbEaLidarProvider, ScotlandLidarProvider, NzLinzProvider, CaHrdemProvider, CzDmrProvider,
    PortugalMDT50cmProvider, PortugalMDT2mProvider,
    JapanTilesProvider, SouthTyrolProvider, EmiliaRomagnaProvider, TinitalyProvider, ItalyVenetoProvider, LombardiaProvider, PiemonteProvider,
...
GB_EA_LIDAR_PROVIDER = GbEaLidarProvider()
SCOTLAND_LIDAR_PROVIDER = ScotlandLidarProvider()
NZ_LINZ_PROVIDER = NzLinzProvider()
CA_HRDEM_PROVIDER = CaHrdemProvider()
CZ_DMR_PROVIDER = CzDmrProvider()
PORTUGAL_MDT50CM_PROVIDER = PortugalMDT50cmProvider()
PORTUGAL_MDT2M_PROVIDER = PortugalMDT2mProvider()
JAPAN_TILES_PROVIDER = JapanTilesProvider()
SOUTH_TYROL_PROVIDER = SouthTyrolProvider()
EMILIA_ROMAGNA_PROVIDER = EmiliaRomagnaProvider()
TINITALY_PROVIDER = TinitalyProvider()
ITALY_VENETO_PROVIDER = ItalyVenetoProvider()
LOMBARDIA_PROVIDER = LombardiaProvider()
PIEMONTE_PROVIDER = PiemonteProvider()
ITALY_FVG_PROVIDER = ItalyFvgProvider()
ITALY_TRENTINO_PROVIDER = ItalyTrentinoProvider()
TOSCANA_1M_PROVIDER = ToscanaDtm1mProvider()
TOSCANA_10M_PROVIDER = ToscanaDtm10mProvider()
BE_FLANDERS_PROVIDER = FlandersDhmProvider()   # DHM Vlaanderen II 1m WCS
BE_WALLONIA_PROVIDER = WalloniaMntProvider()   # Wallonia MNT 50cm (identify + cache)
...
PROVIDERS: Dict[str, ElevationProvider] = {
    ...
    'SI': SloveniaProvider(),     # ARSO ImageServer (remote), 1m, CLSS 2023-25
    'ES': SPAIN_WCS_PROVIDER,     # WCS 2.0.1, 5m, final ES fallback
    'NL': NetherlandsProvider(),  # WCS 2.0.1, 0.5m, confirmed working
    'GB': GB_EA_LIDAR_PROVIDER,   # EA LIDAR 1m (E+W) + Scotland LIDAR 0.5-1m + GPXZ fallback (NI)
    'CA': CA_HRDEM_PROVIDER,      # NRCan HRDEM 1-2m tiles, GPXZ fallback for uncovered
    'CZ': CZ_DMR_PROVIDER,        # ČÚZK DMR 5G 1m WCS 2.0.1, national coverage
    ...
    'AT': AUSTRIA_ALS1_PROVIDER,  # Austria uses ALS1->DGM5 chain
    'BE': BE_FLANDERS_PROVIDER,   # BE uses COUNTRY_PROVIDER_CHAINS (Flanders WCS -> Wallonia identify)
    'AU': GPXZProvider(),         # GPXZ REST, 5m
    'MX': GPXZProvider(),         # GPXZ REST, 5m
    'HK': GPXZProvider(),         # GPXZ REST, 50cm
    'JP': JAPAN_TILES_PROVIDER,   # GSI DEM5A 5m PNG tiles, GPXZ fallback for gap coverage
    'IT': GPXZProvider(),           # Italy — BZ 2.5m chain + GPXZ fallback (TINITALY 10m too coarse for road profiles)
    'NZ': NZ_LINZ_PROVIDER,       # LINZ 1m COGs, no fallback (GPXZ 30m useless)
    'PT': PORTUGAL_MDT50CM_PROVIDER,  # DGT MDT50cm→MDT2m tiles, GPXZ fallback
}

# Use dedicated GPXZ provider instances per country so source tags are accurate.
# Countries with chains (GB, CA, NZ, JP, IT, PT, ES, AT) use COUNTRY_PROVIDER_CHAINS below.
PROVIDERS['GB'] = _gpxz_for('GB')  # EA LIDAR catalog not yet built; GPXZ is active provider
PROVIDERS['AU'] = _gpxz_for('AU')
PROVIDERS['MX'] = _gpxz_for('MX')
PROVIDERS['HK'] = _gpxz_for('HK')
PROVIDERS['PT'] = _gpxz_for('PT')  # DGT tiles inactive by default; GPXZ is active provider
...
COUNTRY_PROVIDER_CHAINS: Dict[str, List[Tuple[str, ElevationProvider]]] = {
    ...
    'GB': [
        ('GB EA LIDAR', GB_EA_LIDAR_PROVIDER),
        ('Scotland LIDAR', SCOTLAND_LIDAR_PROVIDER),
        ('GB GPXZ', _gpxz_for('GB')),
    ],
    'IM': [
        # Isle of Man — Crown dependency. EA LIDAR WCS covers it (OSGB36 ~E220k N480k).
        # Scotland LIDAR northing filter (N≥530k) will skip it cleanly → GPXZ fallback.
        ('GB EA LIDAR', GB_EA_LIDAR_PROVIDER),
        ('IM GPXZ', _gpxz_for('IM')),
    ],
    'CA': [
        ('CA HRDEM', CA_HRDEM_PROVIDER),
        ('CA GPXZ', _gpxz_for('CA')),
    ],
    'JP': [
        ('JP DEM5A', JAPAN_TILES_PROVIDER),
        ('JP GPXZ', _gpxz_for('JP')),
    ],
    'IT': [
        # Highest-resolution regional tiles/services first — chain falls through on None.
        # Local tile providers (TN, FVG, ER, VE) return None quickly for out-of-region
        # points so cross-region requests are cheap. Remote services (BZ WCS, LO/PI ArcGIS,
        # Toscana WMS) are slower on miss but still bounded.
        ('IT TN DTM 0.5m', ITALY_TRENTINO_PROVIDER),
        ('IT FVG DTM 1m', ITALY_FVG_PROVIDER),
        ('IT TO DTM 1m', TOSCANA_1M_PROVIDER),
        ('IT BZ WCS 2.5m', SOUTH_TYROL_PROVIDER),
        ('IT ER DTM 5m', EMILIA_ROMAGNA_PROVIDER),
        ('IT VE DTM 5m', ITALY_VENETO_PROVIDER),
        ('IT LO DTM 5m', LOMBARDIA_PROVIDER),
        ('IT PI DTM 5m', PIEMONTE_PROVIDER),
        ('IT TO DTM 10m', TOSCANA_10M_PROVIDER),
        ('IT TINITALY 10m', TINITALY_PROVIDER),
        ('IT GPXZ', _gpxz_for('IT')),
    ],
    'NZ': [
        ('NZ LINZ', NZ_LINZ_PROVIDER),
    ],
    'PT': [
        ('PT MDT50cm', PORTUGAL_MDT50CM_PROVIDER),
        ('PT MDT2m', PORTUGAL_MDT2M_PROVIDER),
        ('PT GPXZ', _gpxz_for('PT')),
    ],
    'BE': [
        # Flanders WCS (1m) first; it pre-filters to its envelope and returns
        # None for Wallonia, which then resolves via the MapServer-identify
        # provider (50cm). No GPXZ fallback — its Belgium data is only 20m.
        ('BE Flanders DHM 1m', BE_FLANDERS_PROVIDER),
        ('BE Wallonia MNT 50cm', BE_WALLONIA_PROVIDER),
    ],
}
...
NEIGHBOR_COUNTRIES: Dict[str, List[str]] = {
    ...
    'US': ['CA', 'MX'],
    'MX': ['US'],
    # Asia-Pacific — no land border providers yet, GPXZ only
    'JP': [],  # South Korea, Russia, China not yet supported
...
async def _query_provider_chain(
    ...
    for step_i, (label, provider) in enumerate(chain):
        ...
        try:
            ...
            step_error = str(e)
            if verbose or step_i == len(chain) - 1:
                print(f"    [{label}] failed: {e} -> {'no more chain steps' if step_i == len(chain) - 1 else 'continuing fallback chain'}")
...
async def get_all_elevations(
    ...
    for cc, start, end in runs:
        ...
        # Resample local providers to native resolution before querying
        if provider.is_local:
            spacing = getattr(provider, 'local_resample_m', LOCAL_RESAMPLE_M)
            query_pts = _resample_to_spacing(run_pts, spacing)
