...
async def get_all_elevations(
    ...
    if unsupported:
        # Check each unsupported country for a supported neighbour to absorb its points.
        # e.g. IT → ['AT', 'SI', 'FR', 'CH']: near-border points geocoded to Italy get
        # reassigned to the first supported neighbour in the list.
        unresolvable = []
        for cc in sorted(unsupported):
            neighbours = NEIGHBOR_COUNTRIES.get(cc, [])
            supported_nb = next((nb for nb in neighbours if nb in SUPPORTED_COUNTRIES), None)
            if supported_nb:
                print(f"  [remap] {cc} not supported — reassigning border points to {supported_nb}")
            else:
    ...
    # Build per-point country map. Unsupported countries are remapped to the first
    # supported neighbour listed in NEIGHBOR_COUNTRIES.
    point_country: List[str] = [''] * len(points)
    for cc, indexed_points in groups.items():
        effective_cc = cc
        if cc not in SUPPORTED_COUNTRIES:
            neighbours = NEIGHBOR_COUNTRIES.get(cc, [])
            effective_cc = next((nb for nb in neighbours if nb in SUPPORTED_COUNTRIES), None)
            if effective_cc is None:
        ...
        for idx, _ in indexed_points:
            point_country[idx] = effective_cc
