...
const PRIMARY_SERVERS = [
  ...
  // NOTE: maps.mail.ru removed — that mirror has 403'd every request since April 2026.
]
...
function parseOverpassResponse(data) {
  ...
  for (const el of data.elements) {
    ...
    const layer = parseInt(el.tags?.layer, 10) || 0
    result[curType].push({ id: el.id, geometry: valid, name, layer })
  }
  return result
}
...
async function queryOverpass(server, query, requestTimeoutMs = REQUEST_TIMEOUT_MS) {
...
/**
 * Run an arbitrary Overpass QL query through the hardened mirror-fallback + retry +
 * Retry-After backoff loop, returning the parsed JSON. Shared by `fetchBridgesAndTunnels`
 * (the bbox bridge/tunnel query) and the roundabout ring fetcher (a single-way query) so
 * BOTH get the same resilience instead of reimplementing a weaker loop. Throws an
 * aggregated error (`.hasTransient` / `.failures`) only when every mirror+attempt fails.
 *
 * @param {string} query — full Overpass QL (include your own `[timeout:N]`)
 * @param {{ bounds?:object, servers?:Array, onProgress?:Function, progressBase?:number, progressSpan?:number, requestTimeoutMs?:number, attemptsPerServer?:number }} [opts]
 *   bounds: enables the regional mirror for that area; servers: explicit mirror list
 *   (overrides bounds); requestTimeoutMs: per-attempt fetch timeout; attemptsPerServer:
 *   same-server retries (default 3). Use 1 for tiny/cheap queries — retrying a small query
 *   against a server that just 429/504'd only deepens the rate-limit; roll to the next mirror.
 * @returns {Promise<object>} parsed Overpass JSON
 */
export async function runOverpassQuery(query, opts = {}) {
  const servers = opts.servers || getActiveServers(opts.bounds)
  const { onProgress, progressBase = 0, progressSpan = 0 } = opts
  const requestTimeoutMs = opts.requestTimeoutMs ?? REQUEST_TIMEOUT_MS
  const attemptsPerServer = opts.attemptsPerServer ?? ATTEMPTS_PER_SERVER
  const failures = []
  ...
  for (let si = 0; si < servers.length; si++) {
    ...
    for (let attempt = 0; attempt < attemptsPerServer; attempt++) {
      const rel = (si + (attempt / attemptsPerServer)) / Math.max(1, servers.length)
      onProgress?.(
        Math.min(99, Math.round(progressBase + progressSpan * rel)),
        `Overpass: ${server.name} (${attempt + 1}/${attemptsPerServer})`,
      )
      ...
      try {
        return await queryOverpass(server, query, requestTimeoutMs)
      } catch (err) {
        const failure = {
          server: server.name,
          attempt: attempt + 1,
        ...
        if (failure.retryable && attempt < attemptsPerServer - 1) {
...
async function fetchFromMirrors(bounds, onProgress, progressBase, progressSpan) {
  const data = await runOverpassQuery(buildQuery(bounds, QUERY_TIMEOUT_SEC), {
    servers: getActiveServers(bounds), onProgress, progressBase, progressSpan,
  })
