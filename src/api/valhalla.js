 ...
 * @returns {Promise<{ coords: [number, number][], dist: number }>}
 */
async function valhallaRoute(fromLat, fromLon, toLat, toLon, costing = 'auto') {
  const MAX_RETRIES = 4
 ...
 */
export async function valhallaMulti(waypoints, costing = 'auto') {
  const MAX_RETRIES = 4
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    let r
    try {
 ...
 */
export async function valhallaBatchRoute(waypoints, costing = 'auto', onProgress) {
  ...
  for (let b = 0; b < waypoints.length - 1; b += batchSize - 1) {
    ...
    try {
      const segments = await valhallaMulti(batch, costing)
      ...
      for (let i = 0; i < batch.length - 1; i++) {
        try {
          ...
          const [seg] = await valhallaMulti(pair, costing)
 ...
 * @param {number} toLat
 * @param {number} toLon
 * @param {string} [profile='car'] — 'car' (default), 'bike', or 'auto'
 * @returns {Promise<{ coords: [number, number][], dist: number }>}
 *   coords as [lat, lon] pairs, dist in metres
 */
export async function valhallaSegment(fromLat, fromLon, toLat, toLon, profile = 'car') {
  if (profile === 'car') {
    return await valhallaRoute(fromLat, fromLon, toLat, toLon, 'auto')
  }

  if (profile === 'bike') {
    return await valhallaRoute(fromLat, fromLon, toLat, toLon, 'bicycle')
  }
