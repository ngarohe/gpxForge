...
/**
 * POST to Valhalla with fast failure on connection errors and
 * exponential backoff only on genuine 429 rate limits.
 * @param {object} body — request body
 * @returns {Promise<Response>}
 */
async function valhallaFetch(body) {
  ...
  for (let retry = 0; retry <= MAX_429_RETRIES; retry++) {
    ...
    try {
      ...
      // Connection refused / DNS failure — fail immediately, don't retry
      throw new Error('Valhalla: server unreachable')
    }

    if (r.status === 429) {
      const delay = 1000 * Math.pow(2, retry)
      console.warn(`[Valhalla] 429 rate limited, retry ${retry + 1}/${MAX_429_RETRIES} after ${delay}ms`)
...
async function valhallaRoute(fromLat, fromLon, toLat, toLon, costing = 'auto', costingOpts = {}) {
  ...
  const r = await valhallaFetch(body)
 ...
 */
/** OSRM profile to use as fallback for each Valhalla costing */
const OSRM_FALLBACK = { auto: 'car', bicycle: 'bike', pedestrian: 'foot' }
...
export async function valhallaSegment(fromLat, fromLon, toLat, toLon, profile = 'car', opts = {}) {
  ...
  if (profile === 'car' || profile === 'bike' || profile === 'pedestrian') {
    ...
    try {
      return await valhallaRoute(fromLat, fromLon, toLat, toLon, costing, costingOpts)
    } catch (err) {
      // Fallback to OSRM when Valhalla is down
      const osrmProfile = OSRM_FALLBACK[costing]
      console.warn(`[Valhalla] ${profile} failed (${err.message}), falling back to OSRM ${osrmProfile}`)
      return await osrmRoute(fromLat, fromLon, toLat, toLon)
    }
  }

  // 'auto' mode: try OSRM bike first, fallback to Valhalla car, then OSRM car
  try {
    ...
    if (wps) {
      ...
      if (maxSnap > SNAP_DISTANCE_LIMIT) {
        try {
          ...
          return result // OSRM result is still better than nothing
        }
      }
    }
