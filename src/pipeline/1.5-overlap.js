...
// ────────────────────────────────────────────────────────────────────
// Phase 5: Collapse both passes to the road centreline
// ────────────────────────────────────────────────────────────────────

/**
 * For each second-pass point in the segment, find the nearest first-pass point
 * and record the snap target as the MIDPOINT of the pair — i.e. the road
 * centreline. Both passes move there, so a narrow single carriageway ridden in
 * both directions (e.g. out on one lane, back on the other, ~6 m apart) ends up
 * centred on the road rather than collapsed onto one lane. Both passes still
 * share identical XY, preserving the out-and-back elevation-consistency goal.
 *
 * Wider divided / multi-lane roads never reach this code: they exceed the
 * detection `threshold` (default 8 m, the hardcoded single-carriageway max
 * width) and stay separate, as desired.
 *
 * Targets are computed from the original coordinates (before any mutation),
 * so the result is independent of application order.
 *
 * @returns {Array<{secondIdx: number, firstIdx: number, targetLat: number, targetLon: number, distance: number}>}
 */
export function snapOverlapSegment(segment, lats, lons, direction) {
  ...
  for (const sIdx of secondIndices) {
    ...
    if (bestFirst && bestDist <= 15) {
      snaps.push({
        secondIdx: sIdx,
        firstIdx: bestFirst.idx,
        targetLat: (lats[sIdx] + bestFirst.lat) / 2,
        targetLon: (lons[sIdx] + bestFirst.lon) / 2,
        distance: bestDist,
      })
    }
  }
...
/**
 * Mutate lat/lon arrays, moving BOTH the first- and second-pass point of each
 * matched pair onto their shared midpoint (road centreline).
 * @returns {number} count of snapped second-pass points
 */
function applySnaps(snaps, lats, lons) {
  for (const snap of snaps) {
    lats[snap.secondIdx] = snap.targetLat
    lons[snap.secondIdx] = snap.targetLon
    lats[snap.firstIdx] = snap.targetLat
    lons[snap.firstIdx] = snap.targetLon
  }
  return snaps.length
}
...
export function snapOverlaps(lats, lons, opts = {}) {
  // Hardcoded single-carriageway max width (m). Passes closer than this are the
  // same narrow road ridden both ways → collapse to the centreline. Passes
  // farther apart are separate lanes of a divided/multi-lane road → left alone.
