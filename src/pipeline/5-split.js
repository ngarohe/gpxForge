...
/**
 * Generate splits from timing analysis at a target distance.
 * Divides total distance into equal segments, finding the nearest
 * cumulative distance boundary for each split. Mirrors generateSplits()
 * but partitions on cumDist instead of cumTime, returning the same row shape.
 *
 * @param {{ cumTime: number[], cumDist: number[], cumClimb: number[], totalDist: number }} analysis
 * @param {number} targetM — target split distance in metres
 * @returns {Array<{ startIdx: number, endIdx: number, time: number, dist: number, climb: number, avgSpeed: number }>}
 */
export function generateSplitsByDistance(analysis, targetM) {
  const a = analysis
  if (!a || a.totalDist <= 0 || targetM <= 0) return []

  const n = Math.max(1, Math.round(a.totalDist / targetM))
  const adjTarget = a.totalDist / n
  const result = []
  let start = 0

  for (let seg = 0; seg < n; seg++) {
    const endDist = (seg + 1 === n) ? a.totalDist : adjTarget * (seg + 1)
    let endIdx = start

    for (let i = start + 1; i < a.cumDist.length; i++) {
      if (a.cumDist[i] >= endDist) {
        endIdx = Math.abs(a.cumDist[i] - endDist) < Math.abs(a.cumDist[i - 1] - endDist) ? i : i - 1
        break
      }
      if (i === a.cumDist.length - 1) endIdx = i
    }

    if (endIdx <= start) endIdx = Math.min(start + 1, a.cumDist.length - 1)
