...
import { runSmoothing, runSimplify, isLowResSource } from '../pipeline/4-smooth.js'
...
export async function runAutoPipeline(onProgress, opts = {}) {
  ...
  try {
    ...
    // Lazy-fetch OSM way IDs for brunnel parallel-road filter
    if (!ST.routeWayIds || ST.routeWayIds.length !== ST.gpx.lats.length) {
      try {
        const { wayIds, source } = await getRouteWayIds(ST.gpx.lats, ST.gpx.lons)
        ST.routeWayIds = wayIds
        ST.routeWayIdsSource = source
        log(`Way IDs: ${source} (${wayIds.length} pts)`)
      } catch (err) {
        log(`Way IDs: failed — ${err.message}`)
        ST.routeWayIds = null
    ...
    const brunnels = await locateBrunnels(
      ...
      { queryBuffer: 10, routeBuffer: 3, bearingTol: 20, routeWayIds: ST.routeWayIds },
      (pct, msg) => {
  ...
  try {
    ...
    if (ST.brunnels && ST.brunnels.length > 0) {
      const shapeParams = {
        smart: true, tangWin: 8, hermDev: 0.5,
        bridgeDip: 1.0, tunnelSpk: 1.0,
      }
      const osmResult = buildBrunnelCorrections(
        ST.brunnels, ST.gpx.eles, ST.dists, shapeParams,
      )
    ...
    // Auto-accept suspects (runCleaner returns them with accepted=false). Brunnels
    // and artifacts keep whatever runCleaner/buildBrunnelCorrections returned —
    // on low-res sources artifacts come back as accepted=false for review.
  ...
  try {
    ...
    const smoothResult = runSmoothing(
      ...
      { origAvgSpacing: ST.origAvgSpacing, lidarSource: ST.lidarSource, sourceSegments: ST.sourceSegments },
    )
    ...
    ST.smoothedRoute = {
      ...
      origDists: new Float64Array(smoothResult.origDists),
    }
