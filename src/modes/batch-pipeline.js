...
import { isLowResSource } from '../pipeline/4-smooth.js'
...
async function processEntry(entry) {
  ...
  let brunnels = []
  try {
    // Resolve OSM way IDs for parallel-road filter (fail-open on error)
    let routeWayIds = null
    try {
      const result = await getRouteWayIds(lats, lons)
      routeWayIds = result.wayIds
      log(`way IDs: ${result.source}`)
    } catch (wayErr) {
      log(`way IDs failed: ${wayErr.message}`)
    }
    entry.routeWayIds = routeWayIds

    brunnels = await locateBrunnels(
      lats, lons, dists, eles,
      { queryBuffer: 10, routeBuffer: 3, bearingTol: 20, routeWayIds },
      () => {},
    )
  ...
  try {
    ...
    const { gpxText, source, sourceSegments } = await fetchLidarElevations(gpxString, entry.filename)
    ...
    entry.sourceSegments = sourceSegments || []
  ...
  try {
    ...
    if (brunnelList.length > 0) {
      ...
      const osmResult = buildBrunnelCorrections(
        brunnelList, eleWork, entry.dists, shapeParams,
      )
      eleWork = osmResult.eleClean
      osmCorrs = osmResult.corrections
    }

    const autoApplyArtifacts = !isLowResSource(entry.lidarSource)
    ...
    // Auto-accept suspects only. Brunnels stay accepted (from buildBrunnelCorrections);
    // on low-res sources artifacts come back accepted=false from runCleaner.
    for (const c of allCorrs) {
      if (c.type === 'suspect') {
        if (c.interp === 'none') c.interp = 'uniform'
        c.accepted = true
        c.rejected = false
      }
    }
...
/**
 * Build a minimal raw-route snapshot (gpx + dists + grOrig only). All other
 * pipeline outputs are null/empty so a Resume restores the route exactly as
 * uploaded. Mirrors the shape produced by snapshotST() so restoreST() works.
 */
function buildRawSnapshot(gpx, dists, filename) {
  const distsF64 = dists instanceof Float64Array ? dists : new Float64Array(dists)
  const eles = gpx.eles ? gpx.eles.slice() : new Array(gpx.lats.length).fill(0)
  const grOrig = grads(eles, distsF64)
  return {
    gpx: { lats: [...gpx.lats], lons: [...gpx.lons], eles },
    dists: distsF64,
    grOrig,
    eleClean: eles.slice(),
    grClean: grOrig.slice(),
    eleSmoothed: null,
    grSmoothed: null,
    smoothedRoute: null,
    brunnels: null,
    ...
    simplifyStack: [],
    simplifyIdx: -1,
    origAvgSpacing: dists.length > 1 ? dists[dists.length - 1] / (dists.length - 1) : 1,
    lidarSource: '',
    lidarSources: {},
    sourceSegments: [],
  }
 ...
 * @param {{ autoProcess?: boolean }} [opts] - autoProcess=true (default) starts the worker;
 *   when false, files are enqueued as raw parked entries that the user must Process All to run.
 */
export async function addFiles(files, parseGpxFn, opts = {}) {
  ...
  for (const file of sorted) {
    try {
      ...
      const distsF64 = gpx.dists instanceof Float64Array ? gpx.dists : new Float64Array(gpx.dists)
      /** @type {QueueEntry} */
      let entry
      if (autoProcess) {
        entry = {
        ...
        // Expert-mode bulk upload: park raw so the user can edit, then re-park,
        // then click Process All to run LIDAR + brunnels + clean.
        const snapshot = buildRawSnapshot(gpx, distsF64, file.name)
        entry = {
          id: uid(file.name),
          filename: file.name,
          fileSizeBytes: file.size,
...
export function saveBackFull(id, snapshot) {
  ...
  entry.sourceSegments = snapshot.sourceSegments || []
  notify()
}
