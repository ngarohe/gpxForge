...
// Routing flags — defaults match valhalla.js (both ON: snaps recorded
// GPS tracks correctly through one-ways and turn restrictions). Updated
// from main.js when the user toggles the inputs in the builder panel.
let _ignoreOneways = true
let _ignoreRestrictions = true
...
export function getBuilderIgnoreOneways() { return _ignoreOneways }
export function getBuilderIgnoreRestrictions() { return _ignoreRestrictions }

/**
 * Update routing flags used for all subsequent Valhalla calls in builder.
 * Call from main.js on input change and on init from localStorage.
 * @param {{ ignoreOneways?: boolean, ignoreRestrictions?: boolean }} flags
 */
export function setBuilderRoutingFlags(flags = {}) {
  if (typeof flags.ignoreOneways === 'boolean') _ignoreOneways = flags.ignoreOneways
  if (typeof flags.ignoreRestrictions === 'boolean') _ignoreRestrictions = flags.ignoreRestrictions
}
...
export async function onBuilderMapClick(lat, lon) {
  ...
  if (prev) {
    if (_mode === 'routed') {
      ...
      _notify()
      try {
        const result = await valhallaSegment(prev.lat, prev.lon, lat, lon, _profile, { ignoreOneways: _ignoreOneways, ignoreRestrictions: _ignoreRestrictions })
  ...
  try {
    ...
    if (wpIdx > 0) {
      ...
      if (segBefore.type === 'routed') {
        try {
          const r = await valhallaSegment(prev.lat, prev.lon, lat, lon, _profile, { ignoreOneways: _ignoreOneways, ignoreRestrictions: _ignoreRestrictions })
    ...
    if (wpIdx < _waypoints.length - 1) {
      ...
      if (segAfter.type === 'routed') {
        try {
          const r = await valhallaSegment(lat, lon, next.lat, next.lon, _profile, { ignoreOneways: _ignoreOneways, ignoreRestrictions: _ignoreRestrictions })
...
export async function onBuilderDeleteWaypoint(wpIdx) {
  ...
  if (mergedType === 'routed') {
    ...
    _notify()
    try {
      const r = await valhallaSegment(prevWp.lat, prevWp.lon, nextWp.lat, nextWp.lon, _profile, { ignoreOneways: _ignoreOneways, ignoreRestrictions: _ignoreRestrictions })
  ...
  if (origSeg.type === 'routed') {
    ...
    try {
      const [r1, r2] = await Promise.all([
        valhallaSegment(prevWp.lat, prevWp.lon, lat, lon, _profile, { ignoreOneways: _ignoreOneways, ignoreRestrictions: _ignoreRestrictions }),
        valhallaSegment(lat, lon, nextWp.lat, nextWp.lon, _profile, { ignoreOneways: _ignoreOneways, ignoreRestrictions: _ignoreRestrictions }),
      ])
