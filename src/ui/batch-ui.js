...
function buildPanel() {
  ...
  // Left-edge resize handle — drag to widen/narrow the queue panel.
  // Persists chosen width to localStorage so it survives reloads.
  const handle = el('div', 'batch-resize-handle')
  handle.title = 'Drag to resize queue panel'
  let startX = 0, startWidth = 0, dragging = false
  const stored = parseInt(localStorage.getItem('gpxforge.batchPanelWidth') || '', 10)
  if (Number.isFinite(stored) && stored >= 180 && stored <= 600) {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    const next = Math.max(180, Math.min(600, startWidth - (e.clientX - startX)))
  ...
  document.addEventListener('mouseup', () => {
    ...
    localStorage.setItem('gpxforge.batchPanelWidth', String(w))
  })
...
/** Returns array of completed step keys from a snapshot's stepStatus. */
function completedSteps(entry) {
  const status = entry.snapshot?.stepStatus || {}
  return ['trim', 'snap', 'brunnels', 'clean', 'smooth', 'split']
...
function buildEntry(entry) {
  ...
  if (entry.status === 'parked') {
    const done = completedSteps(entry)
    const doneText = done.length
      ? done.map(k => `✓ ${STEP_LABELS[k] || k}`).join(' ')
      : ''
    const stepLabel = entry.parkedAtStep ? (STEP_LABELS[entry.parkedAtStep] || entry.parkedAtStep) : null
    // Prefer the completed-step list if anything's done; otherwise fall back to
    // "Parked at X" or filesize.
    let meta
    if (doneText) meta = doneText
    else if (stepLabel) meta = `Parked at ${stepLabel}`
    else meta = fmtSize(entry.fileSizeBytes)
    info.appendChild(el('div', 'batch-entry-size', { text: meta }))
  } else {
...
export function refreshBatchPanel() {
  ...
  // Spinner shown whenever the worker has anything in flight.
  const rawCount = countRawParked()
  const inProgress = queue.some(e =>
    e.status === 'pending' || e.status === 'brunnels' ||
    e.status === 'lidar'   || e.status === 'cleaning',
  )
  if (_btnProcess) {
    _btnProcess.style.display = ''
    _btnProcess.classList.toggle('busy', inProgress)
    _btnProcess.disabled = rawCount === 0
    if (inProgress && rawCount === 0) {
      _btnProcess.textContent = 'Processing…'
      _btnProcess.title = 'Worker is processing the queue'
    } else if (rawCount === 0) {
