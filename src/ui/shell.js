...
function buildTopbar() {
  ...
  const fileInput = el('input', null, { type: 'file', accept: '.gpx,.txt,.zip', multiple: 'multiple' })
  fileInput.id = 'fileIn'
...
function buildContent() {
  ...
  _infoContents = {}
  for (const stepId of ['trim', 'snap', 'brunnels', 'clean', 'smooth', 'split', 'builder', 'course']) {
...
async function handleFileSelection(files) {
  ...
  for (const file of files) {
    ...
    if (name.endsWith('.zip')) {
      try {
        ...
        for (const [path, entry] of Object.entries(zip.files)) {
          if (entry.dir || !(path.toLowerCase().endsWith('.gpx') || path.toLowerCase().endsWith('.txt'))) continue
    ...
    } else if (name.endsWith('.gpx') || name.endsWith('.txt')) {
      gpxFiles.push(file)
    }
  }
