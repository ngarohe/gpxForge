...
function findMidpointIndex(dists) {
  if (!dists || !dists.length) return 0
  const half = dists[dists.length - 1] / 2
  for (let i = 0; i < dists.length; i++) {
    if (dists[i] >= half) return i
  }
  return Math.floor(dists.length / 2)
}
...
async function doDownload() {
  ...
  try {
    ...
    const dists = useSmoothed ? ST.smoothedRoute.dists : ST.dists
    const midIdx = dists ? findMidpointIndex(dists) : Math.floor(lats.length / 2)
...
function updateButtons() {
  ...
  const stepLabel = ST.activeStep === 'smooth' || ST.activeStep === 'split'
    ? 'final' : (DOWNLOAD_SUFFIXES[ST.activeStep] || '_cleaned').replace('_', '')
  _dlBtn.title = `Download ${stepLabel} GPX`
...
export function initToolbar(container, actions) {
  ...
  _buildCourseBtn = btn('tool-btn tool-btn--build-course', '\u2B22 Build course')
  _buildCourseBtn.title = 'Build a race course from the OSM street network \u2014 draw an area, then click a path'
  _buildCourseBtn.addEventListener('click', () => {
    if (actions.onBuildCourse) actions.onBuildCourse()
  })
  ...
  container.appendChild(_buildCourseBtn)
  ...
  return {
    ...
    setBuildCourseActive(active) {
      if (!_buildCourseBtn) return
      _buildCourseBtn.textContent = active ? '\u2715 Exit Course' : '\u2B22 Build course'
      _buildCourseBtn.classList.toggle('tool-btn--active', active)
    },
  }
}
