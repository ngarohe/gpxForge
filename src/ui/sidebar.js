...
function setActiveStep(stepId) {
  ...
  ST.activeStep = stepId || null
  if (!stepId || !_stepEls[stepId]) return
...
function setStepStatus(stepId, status, text) {
  // Mirror into ST so snapshotST() captures completed steps \u2014 used by the
  // batch panel to show "\u2713 Trim \u2713 Snap" badges under parked entries.
  if (!ST.stepStatus) ST.stepStatus = {}
  if (status === 'none') delete ST.stepStatus[stepId]
  else ST.stepStatus[stepId] = status
...
export function initSidebar(container, opts = {}) {
  ...
  // Course Builder controls container (shown when Course Builder mode is active).
  _courseControlsEl = el('div', 'step-controls')
  _courseControlsEl.dataset.step = 'course'
  _courseControlsEl.style.display = 'none'
  container.appendChild(_courseControlsEl)
  ...
  return {
    ...
    /** Get the Course Builder controls container */
    getCoursePanel() {
      return _courseControlsEl
    },

    /**
     * Show/hide Course Builder mode: hides step tabs + step controls, shows course controls.
    ...
    setCourseActive(active) {
      ...
      _courseControlsEl.style.display = active ? '' : 'none'
      if (!active && ST.activeStep && _controlEls[ST.activeStep]) {
