...
import {
  PAD, GRAD_CLIP, setupCv, visibleRange, makeXp, columnSamplesCached,
...
export function drawGradient(cv) {
  ...
  const sigBase = `${lo}_${hi}_${cw}`

  // Helper to draw a gradient line
  function drawLine(gr, color, lw, alpha, tag) {
    ctx.beginPath()
    ctx.strokeStyle = color
    ctx.lineWidth = lw
    ctx.globalAlpha = alpha
    const hi = Math.min(iHi, gr.length - 1)
    const samples = columnSamplesCached(tag, sigBase, gr, iLo, hi, i => xp(ST.dists[i]), i => gr[i])
    let started = false
  ...
  // ── Lines ──
  if (ST.grClean) drawLine(ST.grOrig, 'rgba(200,60,60,0.5)', 1, 1, 'gradOrig')
  drawLine(ST.grClean || ST.grOrig, '#0077aa', 1.5, 1, 'gradClean')
  ...
  if (ST.smoothedRoute) {
    ...
    ctx.lineWidth = 2
    const sHiC = Math.min(sHi, sg.length - 1)
    const sigSm = `${lo}_${hi}_${cw}_${xScale}`
    const samples = columnSamplesCached('gradSm', sigSm, sg, sLo, sHiC, i => xp(xDist[i] * xScale), i => sg[i])
    let started = false
    for (const i of samples) {
