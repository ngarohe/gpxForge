...
Two algorithms in GPXForge are direct reimplementations of algorithms from
[GPXmagic](https://github.com/peterjamesward/GPXmagic3) by **Peter Ward**,
published under the [CC0 1.0 Universal](https://creativecommons.org/publicdomain/zero/1.0/) licence (public domain dedication).

**Triangle-area simplification** (`src/utils/geometry.js` → `simplifyByArea()`):
For each interior point, compute the area of the 3D triangle formed by the
previous, current, and next points. Select the smallest 20% as removal
candidates, apply an adjacency filter (never remove two consecutive points),
and apply a curvature guard that protects points on curves tighter than 20m
radius. Repeat until no further simplification is desired.

**Gradient averaging** (`src/pipeline/4-smooth.js` → `smoothElevationsByGradient()`):
Average each gradient segment over a fixed window of ±4 neighbours (9-segment
window total), then recompute elevations from the averaged gradients. This is
GPXmagic's "4-point average" algorithm. After 1m resampling the window covers
~8m of real route distance and reduces maximum gradient by ~40% while keeping
mean elevation error under 0.02m.

No Elm source code from GPXmagic was used. Both implementations are independent
JavaScript rewrites based on the published algorithm descriptions.
