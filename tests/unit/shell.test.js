...
describe('initShell', () => {
  ...
  it('builds expected DOM structure inside #app', async () => {
    ...
    // Single upload zone — accepts GPX and ZIP, allows multiple selection
    const drop = topbar.querySelector('.tb-drop')
    expect(drop).toBeTruthy()
    const fileIn = topbar.querySelector('#fileIn')
    expect(fileIn).toBeTruthy()
    expect(fileIn.getAttribute('accept')).toContain('.gpx')
    expect(fileIn.getAttribute('accept')).toContain('.zip')
    expect(fileIn.hasAttribute('multiple')).toBe(true)

    // No separate bulk button — merged into single upload zone
    expect(topbar.querySelector('.tb-bulk')).toBeNull()
    expect(topbar.querySelector('#bulkIn')).toBeNull()

    // Mode toggle exists
    expect(topbar.querySelector('.tb-mode')).toBeTruthy()
    ...
    // Landing state: map-first before file load
    expect(content.dataset.focus).toBe('landing')
  ...
  it('showViews() defaults to chart focus', async () => {
    const { initShell } = await import('../../src/ui/shell.js')
    const shell = initShell(vi.fn())

    shell.showViews()

    const content = document.getElementById('app').querySelector('.content')
    expect(content.dataset.focus).toBe('chart')
  })

  it('showViews("map") sets map focus', async () => {
    const { initShell } = await import('../../src/ui/shell.js')
    const shell = initShell(vi.fn())
...
describe('upload count routing', () => {
  ...
  function makeGpxFile(name = 'route.gpx') {
    return new File(['<gpx></gpx>'], name, { type: 'application/gpx+xml' })
  }

  it('one file triggers onFileLoaded (single-file pipeline)', async () => {
    const onSingle = vi.fn()
    const onBulk = vi.fn()
    const { initShell } = await import('../../src/ui/shell.js')
    initShell(onSingle, onBulk)

    triggerFileInput(document.getElementById('fileIn'), [makeGpxFile()])
  ...
  it('two files triggers onFilesLoaded (bulk queue)', async () => {
    const onSingle = vi.fn()
    const onBulk = vi.fn()
    const { initShell } = await import('../../src/ui/shell.js')
    initShell(onSingle, onBulk)

    triggerFileInput(document.getElementById('fileIn'), [makeGpxFile('a.gpx'), makeGpxFile('b.gpx')])
    await new Promise((r) => setTimeout(r, 10))
  ...
  it('three or more files all pass through to onFilesLoaded', async () => {
    const onSingle = vi.fn()
    const onBulk = vi.fn()
    const { initShell } = await import('../../src/ui/shell.js')
    initShell(onSingle, onBulk)

    triggerFileInput(document.getElementById('fileIn'),
      [makeGpxFile('a.gpx'), makeGpxFile('b.gpx'), makeGpxFile('c.gpx')])
    await new Promise((r) => setTimeout(r, 10))

    expect(onBulk).toHaveBeenCalledTimes(1)
    expect(onBulk.mock.calls[0][0]).toHaveLength(3)
    expect(onSingle).not.toHaveBeenCalled()
  })

  it('non-GPX/ZIP files are silently ignored (no callback fired)', async () => {
    const onSingle = vi.fn()
    const onBulk = vi.fn()
    const { initShell } = await import('../../src/ui/shell.js')
    initShell(onSingle, onBulk)
    ...
    triggerFileInput(document.getElementById('fileIn'), [txtFile])
