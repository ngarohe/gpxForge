...
## Border Fallback Guardrail (Route Context)

- Border routes can contain points geometrically inside one country but operationally tied to a neighboring provider footprint.
- Use route-context remap for unresolved border points before broad fallback chains.
- This avoids long wrong-country fallback walks and reduces false `0 ele` strips near borders (for example FR near IT).
- Keep fallback diagnostics concise; avoid flooding logs with per-point warnings on large tracks.

## Chart Preview Parity (A/B Drag)

- Draw mode drag preview and existing correction anchor drag preview must render with the same interpolation logic used at commit time.
- Any mismatch between preview and committed profile is treated as a regression, because users shape artifacts by trusting the preview.
- Guardrail: preview code path should call the same interpolation helper/path as apply path (no duplicate math branches).

## Smooth Step Guardrail (Repeatable Z-only)

- First Smooth pass remains full process smoothing (XY + Z pipeline as designed).
- Additional Smooth clicks are Z-only passes over the current smoothed route:
  - preserve `lats/lons/dists`
  - update only `eles/gr`
  - invalidate split analysis caches after each extra pass
- When corrections change, force full smooth rebuild before allowing Z-only repeats.

## Smooth Display Guardrail

- In smoothed contexts (expert smooth/split and simple review), the top chart fill edge should follow smoothed profile geometry.
- The clean black line can be hidden in smoothed contexts to avoid dual-profile confusion.
- Mini profile can stay as the reference strip for cross-checking baseline behavior.
